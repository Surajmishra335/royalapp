    ...
    public function store(Request $request)
    {
        ...
        $response = Http::withToken($token)
            ...
            ->post('https://candidate-testing.api.royal-apps.io/api/v2/books', [
                'author' => ['id' => (int) $request->author_id],  // Ensure integer format
                'title' => $request->title,
                'release_date' => "2025-02-11T17:42:04.207Z",
                'description' => $request->description,
                'isbn' => $request->isbn,
                'format' => $request->format,
                'number_of_pages' => (int) $request->number_of_pages // Ensure integer format
            ]);
        Log::info($response);
